...
import requests
import time
from utils import list_to_csv, get_nested_dict

PRICE_CACHE_FILE = 'data/cg_cache.csv'
...
def get_historical_price(symbol: str, date: datetime):
    ...
    price = get_nested_dict(price_data, 'market_data.current_price.usd')
...
def clean_cache():
    ...
    """
    if not os.path.isfile(PRICE_CACHE_FILE):
        return
    ...
    with open(PRICE_CACHE_FILE, 'r') as f:
        for line in f:
            ...
            if [date, symbol] not in cache:
                ...
                print(
                    f'ERROR: Discarding duplicate cache result for {date}|{symbol}|{price}|{source}')
    cache.sort()
    list_to_csv(cache, PRICE_CACHE_FILE)
